...
router.post('/api/posst/commenttodb', (req, res, next) => {
    const values = [req.body.comment, req.body.user_id, req.body.username, req.body.post_id]
    pool.query(`INSERT INTO comments(comment, user_id, autho, post_id, date_created)
                VALUES($1,$2,$3,$4, NOW())`, values, (q_err, q_res)=> {
                    res.json(q_res.rows);
                    console.log(q_err);
                })
...
router.put('/api/put/commenttodb', (req,res,next) => {
    ...
    pool.query(`UPDATE comments SET
                comment=$1, user_id=$2, post_id=$3, author=$4, date_createed=NOW()
...
router.post('/api/post/userprofiletodb', (req, res, next) => {
    ...
    pool.query(`INSERT INTO users(usernae, email, email_verified, date_created)
